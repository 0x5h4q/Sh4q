...
class StorageRepository(Protocol):
    async def save_node(self, node: Node) -> Node:
        """Insert a new node, or merge attributes into an existing one with the same id (same type+value). Returns the resulting node."""
        ...
    ...
    async def save_relationship(self, relationship: Relationship) -> Relationship:
        """Insert a relationship. Re-saving an identical one (same from/type/to) is a no-op, not a duplicate."""
        ...
