...
The first concrete adapter definition is `SubfinderAdapter`. It is intentionally passive: it invokes only `subfinder -silent -d <target>`, parses newline-delimited hostnames, removes duplicates, and redacts the target in execution evidence. It is not registered in the default scan yet. Before enabling it, verify the installed binary path/version and add an offline integration test through the controlled runner; Subfinder's own provider traffic remains outside Python-level request accounting.
