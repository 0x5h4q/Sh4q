...
Non-SQL usability has begun with `sh4q results`: users can list domains, IPs, URLs, and target-filtered failure evidence without writing SQL. Asset rows remain database-global until `scan_run` ownership is implemented; accurate `--latest` and per-scan views must not be faked from timestamps. JSON/CSV exports and an HTML report remain later work. SQLite remains the source of record.
